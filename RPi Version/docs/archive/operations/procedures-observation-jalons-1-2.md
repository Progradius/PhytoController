# Procédures d'observation des jalons 1 et 2 (archive)

**Statut** : archive, figée le 11 septembre 2026. Ne plus suivre ces procédures comme consignes
courantes.
**Jalons** : jalon 1 (« expérience opérateur », watchdog) clos le 28 août 2026 ; jalon 2 (lot
opérateur, PWA et qualité capteurs) clos le 30 août 2026, fenêtre corrective close le 1er septembre
2026.
**Origine** : section « Contrôle post-déploiement » de
[Déploiement et rollback](../../operations/deployment-and-rollback.md), déplacée ici telle quelle.
Seuls les liens relatifs ont été réécrits pour rester valides depuis ce répertoire.

Les chemins `param/…` cités ci-dessous sont ceux de l'époque : depuis la migration des données
vivantes, ces fichiers vivent dans le répertoire posé par `PHYTO_DATA_DIR` — voir
